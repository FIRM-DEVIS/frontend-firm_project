...
const CalendarView = ({ taches, allTaches, onTacheClick, userInfo }) => {
  ...
  // Fonction utilitaire pour créer une date locale sans décalage de fuseau horaire
  ...
  const getPeriodTasks = () => {
    ...
    const periodStart = new Date(periodData[0]);
    periodStart.setHours(0, 0, 0, 0);
    
    const periodEnd = new Date(periodData[periodData.length - 1]);
    ...
    if (timeView === "month") {
      ...
      periodEndTime = new Date(lastDay);
      ...
      periodEndTime = new Date(lastMonth);
      periodEndTime.setHours(23, 59, 59, 999);
    } else {
      periodEndTime = new Date(periodEnd);
      periodEndTime.setHours(23, 59, 59, 999);
    }

    return tachesToDisplay.filter((tache) => {
      // Utiliser la fonction createLocalDate pour éviter les problèmes de fuseau horaire
      const taskStart = createLocalDate(tache.date_debut);
      const taskEnd = createLocalDate(tache.date_fin_prevu);
      
      taskStart.setHours(0, 0, 0, 0);
      taskEnd.setHours(23, 59, 59, 999);

      return taskStart <= periodEndTime && taskEnd >= periodStart;
    });
  };

  // Fonction pour calculer la position et largeur d'une tâche
  const getTaskPosition = (tache) => {
    // Utiliser createLocalDate pour éviter les problèmes de fuseau horaire
    const taskStart = createLocalDate(tache.date_debut);
    const taskEnd = createLocalDate(tache.date_fin_prevu);
    const periodStart = new Date(periodData[0]);
    periodStart.setHours(0, 0, 0, 0);
    ...
    if (timeView === "week") {
      ...
      const diffStart = Math.floor((taskStart - periodStart) / (1000 * 60 * 60 * 24));
      const diffEnd = Math.floor((taskEnd - periodStart) / (1000 * 60 * 60 * 24));
      
      startUnit = Math.max(0, diffStart);
      endUnit = Math.min(periodLength - 1, diffEnd);
      ...
      startUnit = Math.max(0, taskStart.getMonth() - periodStartMonth);
      endUnit = Math.min(periodLength - 1, taskEnd.getMonth() - periodStartMonth);
    }
                ...
                >
                  <div className="task-bar-content">
                    <div className="custom-tooltip">
                      ...
                      <br />
                      Du{" "}
                      {createLocalDate(tache.date_debut).toLocaleDateString(
                        "fr-FR"
                      )}{" "}
                      au{" "}
                      {createLocalDate(tache.date_fin_prevu).toLocaleDateString(
                        "fr-FR"
                      )}
                    </div>
